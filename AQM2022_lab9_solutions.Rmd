...
If you are searching for reseources on how to organize the data for your term paper: Take a look at this blog post Denis Cohen, Cosima Meyer, Marcel and myself wrote last year: [Efficient Data Management in R](https://www.mzes.uni-mannheim.de/socialsciencedatalab/article/efficient-data-r/). The interesting part starts at "Data Management and Data Manipulation" (you know git by now). We introduce the basic tools you need to manage your data using either base R or tidyverse and then walk through one example where we create an analysis data frame from multiple sources. All the code and data is readily available on github so you can work through the code by yourself. **For your term paper:** If you are facing a concrete problem that you cannot solve: Do not hesitate to ask me for help.
...
**Sociodemographic models** argue that these utilities are systematically influenced by factors, such as **income, class, religion, education, age or whether someone lives in an urban or rural area**.
...
for (i in 1:4) {
  lines(incomeScenario, QoImean[i,], col = partycolors[i])
}
...
for (i in 1:4) {
  polygon(
    ...
    col = adjustcolor("lightgrey", alpha = 0.2) ,
    border = NA
  )
}
...
```{r Exercise I} 
# Set up your scenarios
income_seq <- seq(min(df$income), max(df$income), length.out = 10)

n_scenarios <- length(income_seq) # Number of scenarios
